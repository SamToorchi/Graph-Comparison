...
d3.json("content/2018/eurovisNetwork_2018.json").then(function(twitter) {
    ...
    const filteredNodes_18 = nodes_18.reduce((p,c,i) => {
        if(
            nodes_19.some(x => x.id===c.id)
        )
        {
            p.push(c)
        };
    ...
    var filteredNodes_18_top_100 = filteredNodes_18.sort(function(a, b) {
        return d3.descending(+a.friends_count, +b.friends_count);
    }).slice(0, 100);


    const filteredNodes_19_top_100 = nodes_19.reduce((p,c,i) => {
        if(
            filteredNodes_18_top_100.some(x => x.id===c.id)
        )
        {
            p.push(c)
        };
    ...
    const links = [...filteredLinks_18, ...filteredLinks_19].reduce((r, l) => {
        ...
        nodes = [...filteredNodes_18_top_100, ...filteredNodes_19_top_100].reduce((r, n) => {
